...
I started the assignment by designing the pvp version of the game. After much thought, I decided that I would make my board a curricular linked link, which is a linked list when the tail connects to the head. I did this because I thought it would be so much easier to handle sowing (making moves from pit 14 to pit 1+ ). I implemented the game logic including but not excluding making moves, handling extra turns, handling captures, game over checks. Then I had a functioning  PvP game.

I then started to implement alpha-beta pruning. I had to reimplement the logic of the game and set up for alpha-beta pruning by adding some functions like getting the legal moves and a heuristic. For the heuristic, after much trial and tribulation, I ended up doing a weighted sum of  5 concrete evaluations:
